        ...
        <div className="gate-entry" style={{ paddingBottom: 'calc(2rem + env(safe-area-inset-bottom))' }}>
          ...
          <div className="gate-request-wrap">
            ...
            <div className={`gate-request-panel${showRequest ? ' open' : ''}`}>
              <p className="gate-request-desc">
                {lang === 'ar'
                  ? 'رمز الوصول يُمنح للشركاء والمستثمرين والمتعاونين المختارين. تواصل معنا لطلب رمز.'
                  : 'Access keys are granted to selected co-builders, investors, and collaborators. Reach out to request one.'}
              </p>
              <a
                className="gate-request-btn gate-request-btn--whatsapp"
                href={`https://wa.me/966535271122?text=${encodeURIComponent(lang === 'ar' ? 'مرحباً، أريد طلب رمز وصول إلى منصة MomenCrafts.' : "Hi, I'd like to request an access key to MomenCrafts.")}`}
              ...
              >
                ...
                {lang === 'ar' ? 'طلب عبر واتساب' : 'Request via WhatsApp'}
              </a>
              <a
                className="gate-request-btn gate-request-btn--email"
                href={`mailto:hello@momencrafts.com?subject=${encodeURIComponent('Access Key Request — MomenCrafts')}&body=${encodeURIComponent(lang === 'ar' ? 'مرحباً،\n\nأريد طلب رمز وصول إلى منصة MomenCrafts.\n\nالاسم: \nالتخصص: ' : 'Hi,\n\nI\'d like to request an access key to MomenCrafts.\n\nName: \nBackground: ')}`}
                target="_blank" rel="noopener noreferrer"
              >
                ...
                {lang === 'ar' ? 'طلب عبر البريد' : 'Request via Email'}
              </a>
            </div>
          </div>
        </div>
